...
#        zfile.close()
    ...
    def uploadDemo(self,file_path):
       #单文件上传
        ...
        m = MultipartEncoder(
            fields={
                'name':'uploads','age':'100',
                'files':('1.jpg',open('1.jpg','rb'),'application/octet-stream'),
                'files':('2.jpg',open('2.jpg','rb'),'image/jpeg')
            }
        )
        r5=requests.post('http://localhost:8888/uploads',data=m,headers = {'Content-Type':m.content_type})
        #监听上传进度
